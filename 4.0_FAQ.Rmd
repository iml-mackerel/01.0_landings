...
df.hooknet <- ddply(ziffsub,c('hooknet','doy'),summarise,catch=sum(catch,na.rm=T))
...
```{r switch to hooks annual, fig.width=18,fig.height=14}
df.hooknety <- ddply(ziffsub,c('year','hooknet','doy'),summarise,catch=sum(catch,na.rm=T))
